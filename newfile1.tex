...
Note when $a = 0$, $$\frac{dJ}{da}=0.$$ \textcolor{red}{Why? is this equation true?}
\\
...
Also, $A(t)$ is (basically) arbitrary. Why must $f(t) = 0$??} This equation must be satisfied for a function
...
from Calculus.  We can solve for $m$ and $y_0$. \textcolor{red}{How? Solve for slope and then the y intercept.}
...
\[\frac{dL}{dx} = 0.\]
So we have (\textcolor{red}{fix these equations: should be 
a plus coming from the arc length formula})
...
$$\ddot{x}  =0 ,$$
on the interval $(a,b)$. \textcolor{red}{ Why is it true that 
a function with $x'' = 0$ must be a line?}
