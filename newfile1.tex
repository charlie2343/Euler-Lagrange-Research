...
As $A(t)$ is arbitrary, by the fundamental lemma of calculus of variations:
\begin{align*}
...
a function with $x'' = 0$ must be a line?} When $\ddot{x} =0$, $\dot{x}$ is a constant. therefore x has a constant slope. 
...
To solve for M we could do rise over run for teh interval $\frac{x(a)-x(b)}{a-b}$ 
