...
\begin{theorem}
    Let J be a functional such that $J = \int_{t_{1}}^{t_{2}}L(t,f(t),f'(t))$Let $x(t)$ be a function that extremizes J, the following equation must be satified: 
...
Let $x(t)$ be the function that extremizes the functional \textcolor{red}{What are the bounds for the integral?}$$R(x(t)) =
\int_{t_{1}}^{t_{2}} x(t)dt.$$ \textcolor{red}{Why does $x$ even exist.} Let $A(t)$ be an arbitrary function that represents the divergence \textcolor{red}{what do you mean by divergence exactly?}
from the extremized path $x(t)$. Let $A(t_{1})=A(t_{2})=0$. All possible
